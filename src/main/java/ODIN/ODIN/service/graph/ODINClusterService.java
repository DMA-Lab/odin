...
public class ODINClusterService {
    ...
    public void computeClusters_MPBS() {
        ODINVariable.INSTANCE.getClusters().parallelStream().forEach(this::computeCluster_MPBS);

    }

    public void computeClusters_Dijkstra() {
        ODINVariable.INSTANCE.getClusters().parallelStream().forEach(this::computeCluster_Dijkstra);
    ...
    public void computeCluster_MPBS(ODINCluster cluster) {
        DisComputerUtil.MPBS(cluster);
        saveBorderNames(cluster);
    }

    public void computeCluster_Dijkstra(ODINCluster cluster) {
        DisComputerUtil.Dijkstra(cluster);
        saveBorderNames(cluster);
    }
    ...
    private void saveBorderNames(ODINCluster cluster) {
        ...
        //System.out.println("vertexNum="+clusterLinkMap.size()+",borderVertexNum="+borderNames.size());
        GlobalVariable.borderVertexNum.add(borderNames.size());
    ...
    public static double Memory(double i){
        if (String.valueOf(GlobalVariable.MAP_INFO1) == "NY"){
            i = 0.312;
        }
     ...
     */
    public void updateActiveClusters(Set<String> clusterNames, boolean isFirst) throws InterruptedException {
        Trie trie = new Trie();
