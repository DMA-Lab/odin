package ODIN.base.service.api;

import ODIN.base.common.BaseException;
import ODIN.base.domain.GlobalVariable;
import ODIN.base.domain.enumeration.IndexType;
import ODIN.base.service.dto.IndexDTO;
import ODIN.base.service.dto.UpdateDTO;
import ODIN.base.service.dto.result.IndexResultDTO;
import ODIN.base.service.dto.result.ResultDTO;
import ODIN.base.service.factory.ServiceFactory;
import ODIN.base.service.factory.SpringBeanFactory;
import ODIN.base.service.graph.CarService;
import ODIN.base.service.utils.FileUtil;
import ODIN.base.service.utils.MemoryUtil;
import lombok.extern.slf4j.Slf4j;
import org.apache.lucene.util.RamUsageEstimator;
import org.springframework.stereotype.Service;

import java.io.IOException;
import java.util.HashMap;
import java.util.Map;

/**
 * IndexService
 * 2022/3/20 zhoutao
 */
@Slf4j
@Service
public abstract class IndexService implements IBaseService {

    protected IVariableService variableService;

    protected CarService carService;

    protected double buildTime;

    protected double memoryConsumed;

    protected double updateTime;

    /**
     * build index
     *
     * @param indexDTO indexDTO
     */
    public void buildIndex(IndexDTO indexDTO) throws InterruptedException {
        initVariables(indexDTO);
        // record the memory and time
        //long memoryBefore = MemoryUtil.getUsedMemory();
        long startTime = System.nanoTime();

        build();

        buildTime = (System.nanoTime() - startTime);


        memoryConsumed = RamUsageEstimator.sizeOf(GlobalVariable.variable.getVertices()) + RamUsageEstimator.sizeOf(GlobalVariable.variable.getClusters());


        log.info("Build Done!");
    }

    public void buildIndex_MPBS(IndexDTO indexDTO) throws InterruptedException {
        initVariables(indexDTO);
        // record the memory and time
        long memoryBefore = MemoryUtil.getUsedMemory();
        long startTime = System.nanoTime();

        build_MPBS();

        buildTime = (System.nanoTime() - startTime);

        memoryConsumed = RamUsageEstimator.sizeOf(GlobalVariable.variable.getVertices()) + RamUsageEstimator.sizeOf(GlobalVariable.variable.getClusters());

        log.info("Build Done!");
    }

    public void buildIndex_Dijkstra(IndexDTO indexDTO) throws InterruptedException {
        initVariables(indexDTO);
        // record the memory and time
        long memoryBefore = MemoryUtil.getUsedMemory();
        long startTime = System.nanoTime();

        build();

        buildTime = (System.nanoTime() - startTime);

        memoryConsumed = RamUsageEstimator.sizeOf(GlobalVariable.variable.getVertices()) + RamUsageEstimator.sizeOf(GlobalVariable.variable.getClusters());

        log.info("Build Done!");
    }

    /**
     * init Variable and read files
     *
     * @param index indexDTO
     */
    private void initVariables(IndexDTO index) {
        variableService = ServiceFactory.getVariableService();
        variableService.initVariable(index);
        carService = ServiceFactory.getCarService();
        try {
            SpringBeanFactory.getBean(FileUtil.class).readFiles();
        } catch (IOException | BaseException ioException) {
           // log.error("Read File Error！ " + ioException);
        } catch (InterruptedException e) {
            e.printStackTrace();
        } finally {
           // log.info("vertex Num={}", GlobalVariable.variable.getVerticeSize());
           log.info("cluster Num={}", GlobalVariable.variable.getClusterSize());
        }
    }

    /**
     * build index in details
     */
    protected abstract void build() throws InterruptedException;
    protected abstract void build_MPBS() throws InterruptedException;
    protected abstract void build_Dijkstra() throws InterruptedException;
    /**
     * update car location
     *
     * @param updateDTO updateDTO
     */
    public void updateCar(UpdateDTO updateDTO) throws InterruptedException {
        carService.batchUpdateRandomCarLocation(updateDTO);
        updateTime = System.nanoTime();

        update();

        updateTime = System.nanoTime() - updateTime;
        log.info("Update Done!");
    }

    /**
     * updateIndex
     */
    protected abstract void update() throws InterruptedException;

    /**
     * build Result
     *
     * @param indexDTO indexDTO
     */
    public ResultDTO buildResult(IndexDTO indexDTO) throws InterruptedException {
        IndexResultDTO resultDTO = new IndexResultDTO();
        resultDTO.buildResult(buildTime, memoryConsumed, indexDTO);

        return resultDTO;
    }

    public abstract IndexType supportType();

    public void register() {
        ServiceFactory.register(supportType(), this);
    }



    public static void main(String[] args) {
        Map<String, String> map = new HashMap<>();
        ;
        System.out.println(RamUsageEstimator.sizeOf(map));
    }

}
